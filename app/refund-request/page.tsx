...
export default async function RefundRequestPage({
  ...
  const { token, order } = await searchParams; // ← await it
  ...
  if (
    orderDoc.refundStatus === "approved" ||
    orderDoc.refundStatus === "rejected"
  ) {
    return (
      <div
        style={{
      ...
      >
        ...
        <p style={{ color: "#666" }}>
          ...
          Please reply to your order confirmation email if you have further
          questions.
        </p>
      </div>
    );
  ...
  // Valid — show form
  return <RefundRequestForm orderId={order} token={token} />;
}